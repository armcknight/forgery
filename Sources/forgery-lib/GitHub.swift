...
public extension GitHub {
    ...
    func cloneForUser(basePath: String, repoTypes: RepoTypeOptions.Resolved, organization: String?, dedupeOrgReposCreatedByUser: Bool) async throws {
        ...
        if !repoTypes.noNonstarredRepos {
            ...
            for repo in repos {
                ...
                do {
                    try await cloneRepoType(repo: repo, paths: userPaths.commonPaths, repoTypes: repoTypes)
                } catch {
        ...
        if !repoTypes.noStarredRepos {
            ...
            for repo in repos {
                ...
                do {
                    try await cloneNonForkedRepo(repo: repo, cloneRoot: "\(userPaths.starredRepoPath)/\(owner)", noWikis: repoTypes.noWikis)
                } catch {
        ...
        if !repoTypes.noNonstarredGists {
            ...
            for gist in gists {
                do {
                    ...
                    try await cloneGistType(gist: gist, paths: userPaths, repoTypes: repoTypes)
                } catch {
        ...
        if !repoTypes.noStarredGists {
            ...
            for gist in gists {
                ...
                try await cloneNonForkedGist(gist: gist, cloneRoot: "\(userPaths.starredGistPath)/\(owner)")
            }
        }
    }
    
    func cloneForOrganization(basePath: String, repoTypes: RepoTypeOptions.Resolved, organization: String) async throws {
        ...
        for repo in repos {
            do {
                try await cloneRepoType(repo: repo, paths: orgPaths, repoTypes: repoTypes)
            } catch {
                logger.error("Failed to clone repo \(String(describing: repo.fullName)): \(error)")
            }
        }
    ...
    func tagRepo(repo: Repository, clonePath: String, clearFirst: Bool = false) async throws {
        guard let owner = repo.owner.login else {
        ...
        if clearFirst {
            let currentTags = await shell("tag --no-name \(clonePath)")
            let _ = await shell("tag -r \"\(currentTags)\"")
        }
        let newTags = mutableTopicList.joined(separator: ",")
        let _ = await shell("tag -a \"\(newTags)\" \(clonePath)")
    }
    
    func cloneWiki(repo: Repository, cloneRoot: String) async {
        ...
        if hasWiki {
            ...
            if await remoteRepoExists(repoSSHURL: wikiURL) {
                logger.info("Cloning \(wikiURL)...")
                do {
                    try await Git(path: cloneRoot).run(.clone(url: wikiURL))
                } catch {
    ...
    func cloneNonForkedRepo(repo: Repository, cloneRoot: String, noWikis: Bool) async throws {
        guard let name = repo.name else {
        ...
        try await cloneRepo(repoName: name, sshURL: sshURL, cloneRoot: cloneRoot)
        try await tagRepo(repo: repo, clonePath: "\(cloneRoot)/\(name)")
        
        if !noWikis {
            await cloneWiki(repo: repo, cloneRoot: cloneRoot)
        }
    }
    
    func cloneForkedRepo(repo: Repository, forkPath: String, noWikis: Bool) async throws {
        guard let owner = repo.owner.login else {
        ...
        try await cloneRepo(repoName: repoName, sshURL: sshURL, cloneRoot: cloneRoot)
        let repoPath = "\(cloneRoot)/\(repoName)"
        let git = Git(path: repoPath)
        logger.info("Renaming origin to fork.")
        try await git.run(.renameRemote(oldName: "origin", newName: "fork"))
        guard let parentURL = parentRepo.sshURL else {
            throw ForgeryError.Clone.Repo.noForkParentSSHURL
        }
        guard await remoteRepoExists(repoSSHURL: parentURL) else {
            throw ForgeryError.Clone.Repo.couldNotFetchRepo
        }
        logger.info("Adding upstream remote.")
        try await git.run(.addRemote(name: "upstream", url: parentURL))
        logger.info("Setting default branch.")
        try await setDefaultForkBranchRemotes(git)
        try await tagRepo(repo: parentRepo, clonePath: repoPath)
        if !noWikis {
            await cloneWiki(repo: parentRepo, cloneRoot: cloneRoot)
        }
    }
    
    func cloneRepoType(repo: Repository, paths: CommonPaths, repoTypes: RepoTypeOptions.Resolved) async throws {
        if repo.isFork {
            if repoTypes.noForkedRepos { return }
            try await cloneForkedRepo(repo: repo, forkPath: paths.repoPaths.forkPath, noWikis: repoTypes.noWikis)
        } else {
            if repo.isPrivate {
                if repoTypes.noPrivateRepos { return }
                try await cloneNonForkedRepo(repo: repo, cloneRoot: paths.repoPaths.privatePath, noWikis: repoTypes.noWikis)
            } else {
                if repoTypes.noPublicRepos { return }
                try await cloneNonForkedRepo(repo: repo, cloneRoot: paths.repoPaths.publicPath, noWikis: repoTypes.noWikis)
            }
        }
    }
    ...
    func updateLocalReposUnder(path: String, remoteRepoList: [Repository], pushToForkRemotes: Bool, prune: Bool, pullWithRebase: Bool, pushAfterRebase: Bool, rebaseSubmodules: Bool, publicRepos: Bool = false, privateRepos: Bool = false, forked: Bool = false, gist: Bool = false, starred: Bool = false) async {
        ...
        for repo in try! FileManager.default.contentsOfDirectory(atPath: path) {
            ...
            if forked {
                do {
                    try await git.run(.fetch(remote: "fork"))
                    try await git.run(.pull(remote: "fork", rebase: pullWithRebase))
                    try await git.run(.fetch(remote: "upstream"))
                    try await git.run(.pull(remote: "upstream", rebase: pullWithRebase))
                    if pushToForkRemotes {
                        try await git.run(.push(remote: "fork"))
                    }
                } catch {
                ...
                do {
                    try await git.run(.fetch(remote: "origin"))
                    try await git.run(.pull(remote: "origin", rebase: pullWithRebase))
                    if pushAfterRebase {
                        try await git.run(.push(remote: "origin"))
                    }
                } catch {
            ...
            do {
                try await tagRepo(repo: repoToSync, clonePath: repoPath, clearFirst: true)
            } catch {
                logger.error("Failed to tag repo with GitHub topics")
            }
            
            do {
                try await git.run(.submoduleUpdate(init: true, recursive: true, rebase: rebaseSubmodules))
            } catch {
    ...
    func cloneGistType(gist: Gist, paths: UserPaths, repoTypes: RepoTypeOptions.Resolved) async throws {
        ...
        if fullyFetchedGist.forkOf != nil {
            ...
            try await cloneForkedGist(gist: fullyFetchedGist, forkPath: paths.forkedGistPath)
        } else {
            ...
            if isPublic {
                ...
                try await cloneNonForkedGist(gist: fullyFetchedGist, cloneRoot: paths.commonPaths.gistPaths.publicPath)
            } else {
                if repoTypes.noPrivateGists { return }
                try await cloneNonForkedGist(gist: fullyFetchedGist, cloneRoot: paths.commonPaths.gistPaths.privatePath)
            }
        }
    }
    
    func cloneForkedGist(gist: Gist, forkPath: String) async throws {
        guard let id = gist.id else {
            throw ForgeryError.Clone.Gist.noID
        }
        ...
        try await cloneRepo(repoName: gistTitle, sshURL: pullURL.absoluteString, cloneRoot: cloneRoot)
        ...
        guard await remoteRepoExists(repoSSHURL: parentURL.absoluteString) else {
            throw ForgeryError.Clone.Gist.couldNotFetchForkParent
        }
        logger.info("Adding upstream remote.")
        try await git.run(.addRemote(name: "upstream", url: parentURL.absoluteString))
        logger.info("Setting default branch.")
        try await setDefaultForkBranchRemotes(git)
    }
    
    func cloneNonForkedGist(gist: Gist, cloneRoot: String) async throws {
        ...
        try await cloneRepo(repoName: fileName, sshURL: pullURL.absoluteString, cloneRoot: cloneRoot)
    }
}
